...
from crud import create_user, list_users, delete_user, record_session, list_sessions
...
async def post_admin_login(
    ...
    # hard-coded single admin account
    ...
    # set a simple cookie for admin auth
    resp.set_cookie("admin", "true", httponly=True)
    return resp

# ---- Protected admin panel (static HTML) ----
...
async def api_list_users(current=Depends(get_admin_user)):
    return list_users(SessionLocal())

@app.post("/admin/users", response_model=UserOut)
async def api_create_user(
    u: UserCreate, current=Depends(get_admin_user)
):
    return create_user(SessionLocal(), u)

@app.delete("/admin/users/{user_id}", response_model=UserOut)
async def api_delete_user(
    user_id: int, current=Depends(get_admin_user)
):
...
async def api_list_sessions(current=Depends(get_admin_user)):
    return list_sessions(SessionLocal())
